...
import com.gmail.merikbest.ecommerce.dto.user.BaseUserResponse;
import com.gmail.merikbest.ecommerce.dto.user.UserResponse;
import com.gmail.merikbest.ecommerce.mapper.OrderMapper;
import com.gmail.merikbest.ecommerce.mapper.PerfumeMapper;
...
public class AdminController {
    ...
    @PostMapping(ADD)
    public ResponseEntity<FullPerfumeResponse> addPerfume(@RequestPart(name = "file", required = true)MultipartFile file,
    ...
    @GetMapping(ORDERS)
    public ResponseEntity<List<OrderResponse>> getAllOrders(@PageableDefault(size = 10) org.springframework.data.domain.Pageable pageable) {
    ...
    public ResponseEntity<List<OrderResponse>> getUserOrdersByEmail(@PathVariable String userEmail,
        ...
        HeaderResponse<OrderResponse> response = orderMapper.getUserOrders(userEmail, (org.springframework.data.domain.Pageable) pageable);
        return ResponseEntity.ok().headers(response.getHeaders()).body(response.getItems());
    }
    ...
    public ResponseEntity<List<BaseUserResponse>> getAllUsers(@PageableDefault(size = 10) Pageable pageable) {
        HeaderResponse<BaseUserResponse> response = userMapper.getAllUsers((org.springframework.data.domain.Pageable) pageable);
        return ResponseEntity.ok().headers(response.getHeaders()).body(response.getItems());
    }
    ...
    public ResponseEntity<ExecutionResult> getAllUserByQuery(@RequestBody GraphQLRequest request) {
        return ResponseEntity.ok(graphQLProvider.getGraphQL().execute(request.getQuery()));
    }
